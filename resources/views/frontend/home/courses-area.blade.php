...
<section class="course-area pb-120px">
    <div class="container">
        ...
        <ul class="nav nav-tabs generic-tab justify-content-center pb-4" id="myTab" role="tablist">
            <li class="nav-item">
                <a class="nav-link" id="business-tab" data-toggle="tab" href="#business" role="tab" aria-controls="business" aria-selected="true">Tất cả khóa học</a>
            </li>
        ...
        <div class="card card-item card-preview" data-tooltip-content="#tooltip_content_1{{ $course->id }}">
            <div class="card-image">
                <a href="{{ url('course/details/'.$course->id.'/'.$course->course_name_slug) }}" class="d-block">
                    <img class="card-img-top lazy" src="{{ asset($course->course_image) }}" data-src="images/img8.jpg" alt="Ảnh khóa học ...">
                </a>
        ...
        @endphp
          ...
          <p class="card-text"><a href="{{ route('instructor.details',$course->instructor_id) }}">{{ $course['user']['name'] }}</a></p>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    @else
                    <p class="card-price text-black font-weight-bold">{{ number_format($course->discount_price, 0, '.', ',') }}<sup>₫</sup> <span class="before-price font-weight-medium">{{ number_format($course->selling_price, 0, '.', ',') }}<sup>₫</sup></span></p>
                    @endif



            <div class="icon-element icon-element-sm shadow-sm cursor-pointer" title="Thêm vào yêu thích" id="{{ $course->id }}" onclick="addToWishList(this.id)" ><i class="la la-heart-o"></i></div>
                </div>
            </div><!-- end card-body -->
        </div><!-- end card -->
    </div><!-- end col-lg-4 -->
        ...
        <div class="col-lg-4 responsive-column-half">
         ...
         <h6 class="ribbon ribbon-blue-bg fs-14 mb-3">{{ $course->label }}</h6>
          ...
          <p class="card-text"><a href=" ">{{ $course['user']['name'] }}</a></p>
                    ...
                    @if ($course->discount_price == NULL)
                    <p class="card-price text-black font-weight-bold">{{ $course->selling_price }}<sup>₫</sup> </p>
                    @else
                    <p class="card-price text-black font-weight-bold">{{ $course->discount_price }}<sup>₫</sup> <span class="before-price font-weight-medium">${{ $course->selling_price }}<sup>₫</sup></span></p>
                    @endif

                        <div class="icon-element icon-element-sm shadow-sm cursor-pointer" title="Thêm vào yêu thích"><i class="la la-heart-o"></i></div>
                    </div>
                </div><!-- end card-body -->
            </div><!-- end card -->
        </div><!-- end col-lg-4 -->
        ...
        @endforelse
           ...
           @endforeach
            ...
            <div class="more-btn-box mt-4 text-center">
                <a href="{{ url('course/all') }}"  class="btn theme-btn">Tất cả các khóa học <i class="la la-arrow-right icon ml-1"></i></a>
...
<div class="tooltip_templates">
        ...
        <div class="card card-item">
            <div class="card-body">
                ...
                <div class="d-flex align-items-center pb-1">
                    ...
                    <p class="text-success fs-14 font-weight-medium">Cập nhật<span class="font-weight-bold pl-1">{{ $item->created_at->format('d M Y') }}</span></p>
                </div>
    ...
    @endphp
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <button type="submit" class="btn theme-btn flex-grow-1 mr-3" onclick="addToCart({{ $item->id }}, '{{ $item->course_name }}','{{ $item->instructor_id }}','{{ $item->course_name_slug }}' )" ><i class="la la-shopping-cart mr-1 fs-18"></i>Đăng ký ngay</button>
